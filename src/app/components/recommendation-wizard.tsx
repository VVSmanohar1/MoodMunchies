...
const moods = ["Happy", "Sad", "Stressed", "Adventurous", "Relaxed", "Celebratory"];
const occasions = ["Casual Meal", "Celebration", "Quick Bite", "Date Night", "Family Dinner"];
const cuisines = ["Any", "Italian", "Mexican", "Indian", "Japanese", "Chinese", "American"];
      ...
      <Card className="shadow-lg border-2 border-primary/20">
        ...
        <CardContent className="space-y-6">
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
            <div>
              ...
              <Select name="mood" required>
                <SelectTrigger id="mood"><SelectValue placeholder="Select a mood" /></SelectTrigger>
                <SelectContent>
            ...
            <div>
              ...
              <Select name="occasion" required>
                <SelectTrigger id="occasion"><SelectValue placeholder="Select an occasion" /></SelectTrigger>
                <SelectContent>
                  {occasions.map(occasion => <SelectItem key={occasion} value={occasion.toLowerCase()}>{occasion}</SelectItem>)}
                </SelectContent>
              </Select>
            </div>
          </div>
          <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
            <div>
              ...
              <Select name="cuisine" defaultValue="any" required>
                <SelectTrigger id="cuisine"><SelectValue placeholder="Select a cuisine" /></SelectTrigger>
                <SelectContent>
                  {cuisines.map(cuisine => <SelectItem key={cuisine} value={cuisine.toLowerCase()}>{cuisine}</SelectItem>)}
                </SelectContent>
              </Select>
            </div>
            <div>
              ...
              <Select name="time" required>
                ...
                <SelectContent>
                  {times.map(time => <SelectItem key={time} value={time.toLowerCase()}>{time}</SelectItem>)}
                </SelectContent>
              </Select>
            </div>
            <div>
              <Label htmlFor="location">Your location?</Label>
              <Input id="location" name="location" placeholder="e.g., Downtown, San Francisco" required />
            </div>
          </div>
          <div>
            <Label>Any dietary preferences?</Label>
            <RadioGroup name="dietaryPreference" defaultValue="non-vegetarian" className="mt-2 flex flex-wrap gap-4">
              {dietaryPreferences.map(pref => (
                <div key={pref.value} className="flex items-center space-x-2">
                  <RadioGroupItem value={pref.value} id={pref.value} />
                  <Label htmlFor={pref.value} className="font-normal">{pref.label}</Label>
                </div>
              ))}
            </RadioGroup>
          </div>
          <div>
            ...
            <Textarea id="additionalNotes" name="additionalNotes" placeholder="e.g., 'Looking for a quiet place', 'Something spicy!'" />
          </div>
        </CardContent>
      </Card>
  ...
  return (
    <>
      <div className="flex items-center justify-between">
        <Button type="button" variant="outline" size="lg" onClick={() => toast({ title: "Coming Soon!", description: "Voice input feature is under development." })}>
          <Mic /> Use Voice
        </Button>
...
function EmptyState() {
  return (
    <div className="flex flex-col items-center justify-center rounded-lg border-2 border-dashed border-muted bg-card/50 p-12 text-center animate-in fade-in-50">
      ...
      <h3 className="mt-4 text-xl font-semibold font-headline">No Matches Found</h3>
      <p className="mt-2 text-muted-foreground">
        We couldn't find any recommendations for your criteria. Try being a bit more general!
      </p>
    </div>
  );
}
